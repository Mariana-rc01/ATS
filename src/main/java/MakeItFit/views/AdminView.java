...
public class AdminView extends MakeItFitView {
    ...
    private Menu createAppMenu() {
        ...
        appMenuItems.add(new MenuItem("Create user", () -> {
            try {
                setNewEmail();
                createUser();
            } catch (Exception e) {}
        ...
        appMenuItems.add(new MenuItem("User Details", () -> {
            try {
                setEmail();
                getUserDetails();
            } catch (Exception e) {}
        ...
        appMenuItems.add(new MenuItem("Add activity to user", () -> {
            try {
                setEmail();
                addActivityToUser();
            } catch (Exception e) {}
        }));
        appMenuItems.add(new MenuItem("Remove activity from user", () -> {
            try {
                setEmail();
                removeActivityFromUser();
            } catch (Exception e) {}
        }));
        appMenuItems.add(new MenuItem("List activities from user", () -> {
            try {
                setEmail();
                listActivities();
            } catch (Exception e) {}
        }));
        appMenuItems.add(new MenuItem("Add training plan to user", () -> {
            try {
                setEmail();
                createTrainingPlan();
            } catch (Exception e) {}
        }));
        appMenuItems.add(new MenuItem("Remove training plan", () -> {
            try {
                setEmail();
                removeTrainingPlan();
            } catch (Exception e) {}
        ...
        appMenuItems.add(new MenuItem("List training plans from user", () -> {
            try {
                setEmail();
                listAllTrainingPlansFromUser();
            } catch (Exception e) {}
        }));
     ...
     * Requests the user's email or code from the console and removes the user using the user
     * controller.
     */
    ...
    public Menu createQueryMenu() {
        ...
        queryMenuItems.add(
            new MenuItem("How many kms has a user has run in a period or ever since?",
                         () -> executeQueryHowManyKMsDone()));
        queryMenuItems.add(new MenuItem(
            "How many meters of altimetry has a user totaled in a period or ever since?",
            () -> executeQueryHowManyAltimetryDone()));
        queryMenuItems.add(new MenuItem(
            "What is the most demanding training plan depending on the proposed calorie expenditure?",
            () -> executeQueryMostDemandingTrainingPlan()));
        queryMenuItems.add(new MenuItem("what type of activity is most carried out?",
                                        () -> executeQueryMostDoneActivity()));
        queryMenuItems.add(
            new MenuItem("Which user has burned the most calories over a period of time or ever?",
                         () -> executeQuerywhoBurnsMoreCalories()));
        queryMenuItems.add(new MenuItem(
            "Which user has carried out the most activities in a period or ever since?",
            () -> executeQueryWhoDidTheMostActivities()));
        queryMenuItems.add(new MenuItem("List all activities from a user.",
                                        () -> executeQueryGetAllActivitiesFromUser()));

        return new Menu(queryMenuItems);
    }

    /**
     * Executes the query to determine how many kilometers a user has driven in a period or ever
     * since.
     */
    public void executeQueryHowManyKMsDone() {
        ...
        try {
            ...
            if (response.equals("Y") || response.equals("y")) {
                ...
                System.out.println(
                    "Result: " +
                    this.makeItFitController.executeQueryHowManyKMsDone(startDate, endDate) +
                    " kilometers");
            } else {
                System.out.println(
    ...
    /**
     * Executes the query to determine how many meters of altimetry a user has totaled in a period
     * or ever since.
     */
    public void executeQueryHowManyAltimetryDone() {
        ...
        try {
            ...
            if (response.equals("Y") || response.equals("y")) {
                ...
                System.out.println(
                    "Result: " +
                    this.makeItFitController.executeQueryHowManyAltimetryDone(startDate, endDate) +
                    " meters");
            } else {
                System.out.println(
                    "Result: " + this.makeItFitController.executeQueryHowManyAltimetryDone() +
                    " meters");
            }
        } catch (Exception e) {
            System.out.println("[" + this.makeItFitController.getName() + "] Invalid input.");
    ...
    /**
     * Executes the query to determine the most demanding training plan depending on the proposed
     * calorie expenditure.
     */
    ...
    public void executeQueryMostDoneActivity() {
        ...
        try {
            System.out.println("Result: " +
                               this.makeItFitController.executeQueryMostDoneActivity());
        } catch (Exception e) {
            System.out.println("[" + this.makeItFitController.getName() + "] Invalid input.");
        }
    }
    ...
    /**
     * Executes the query to determine which user has burned the most calories over a period of time
     * or ever.
     */
    ...
    /**
     * Executes the query to determine which user has carried out the most activities in a period or
     * ever since.
     */
    public void executeQueryWhoDidTheMostActivities() {
        ...
        try {
            ...
            if (response.equals("Y") || response.equals("y")) {
                ...
                System.out.println(
                    "Result: \n" +
                    this.makeItFitController.executeQueryWhoDidTheMostActivities(startDate,
                                                                                 endDate));
            } else {
                System.out.println("Result: \n" +
