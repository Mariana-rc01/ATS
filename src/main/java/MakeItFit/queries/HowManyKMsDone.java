...
/**
 * This class is responsible for the query that calculates the total distance done by a user in a
...
public class HowManyKMsDone implements Serializable {
     ...
     */
    public double executeQuery(UserManager   userManager,
                               String        email,
                               MakeItFitDate date1,
                               MakeItFitDate date2) {
        ...
        if (date1 != null && date2 != null && date2.isAfter(date1)) {
            ...
            for (Activity a : user.getListActivities()) {
                if (a instanceof Distance && a.getRealizationDate().isAfter(date1) &&
                    a.getRealizationDate().isBefore(date2)) {
