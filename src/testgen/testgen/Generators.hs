...
instance Arbitrary User where
  arbitrary = do
    ...
    case tag of
      "Amateur" -> return $ Amateur code name age gender weight height bpm level address phone email index activities
      "Occasional" -> do
        freq <- elements [1..14]
        return $ Occasional code name age gender weight height bpm level address phone email freq index activities
      "Professional" -> do
        freq <- elements [1..21]
        spec <- genSpecialization
        return $ Professional code name age gender weight height bpm level address phone email freq spec index activities
  ...
  arbitrary = do
    ...
    case tag of
      ...
      "Trail" -> do
         ...
         trailtype <- arbitrary
         return $ Trail userCode activityCode date expDur designation name spec dur cal dist elevGain elevLoss trailtype
      "WeightSquat" -> do
         reps <- elements [5..30]
         series <- elements [1..8]
         weight <- choose (10.0, 200.0)
         return $ WeightSquat userCode activityCode date expDur designation name spec dur cal reps series weight
