...
public class MostDoneActivityTest {
    ...
    private User createUser(String email, List<Activity> activities) {
        User user = new Professional("JohnDoe",
                                     21,
                                     Gender.Male,
                                     59,
                                     150,
    ...
    void testSetup() {
        ...
        this.pushUp      = new PushUp(UUID.randomUUID(),
                                 ...
                                 10,
                                 100);
        this.running     = new Running(UUID.randomUUID(),
                                   MakeItFitDate.of(2000, 1, 1),
                                   70,
                                   "Running",
                                   "For my life",
                                   100,
                                   1);
        this.trail       = new Trail(UUID.randomUUID(),
                               MakeItFitDate.of(2000, 1, 1),
                               120,
                               "Trail",
                               "RunningUpThatHill",
                               ...
                               321,
                               2);
        this.weightSquat = new WeightSquat(UUID.randomUUID(),
                                           MakeItFitDate.of(2000, 1, 1),
                                           45,
                                           "WeightSquat",
                                           "PumpItUp",
    ...
    void testExecuteQuerySingleUserReturnsRepetitions() {
        User user = createUser(
            "jd@uminho.pt",
            Arrays.asList(this.pushUp, this.pushUp, this.running, this.trail, this.weightSquat));
    ...
    void testExecuteQuerySingleUserReturnsDistance() {
        User user = createUser(
            "jd@uminho.pt",
            Arrays.asList(this.pushUp, this.running, this.running, this.trail, this.weightSquat));
        this.userManager.insertUser(user);
    ...
    void testExecuteQuerySingleUserReturnsDistanceWithAltimetry() {
        User user = createUser(
            "jd@uminho.pt",
            Arrays.asList(this.pushUp, this.running, this.trail, this.trail, this.weightSquat));
        this.userManager.insertUser(user);
    ...
    void testExecuteQuerySingleUserReturnsRepetitionsWithWeights() {
        User user = createUser("jd@uminho.pt",
                               Arrays.asList(this.pushUp,
                                             this.running,
                                             this.trail,
                                             this.weightSquat,
                                             this.weightSquat));
    ...
    void testExecuteQueryMultipleUsersReturnsDistance() {
        ...
        User user4 =
            createUser("jd@vice.pt", Arrays.asList(this.running, this.running, this.running));
